...
class InboxVerifier:
    ...
    async def __get_signature_from_kv(
        ...
        Optional[RSAPublicKey | Ed25519PublicKey | ec.EllipticCurvePublicKey],
        bool,
    ]:
        cache = False
    ...
    async def __save_signature_to_kv(self, key_id: str, public_key: Any) -> None:
        if isinstance(
            public_key,
            (
                rsa.RSAPublicKey,
    ...
    async def __verify_rfc9421(
        ...
        async def _verify_with_key(
            ...
            try:
                ...
                if verified_key_id:
                    if not is_cache:
                        await self.__save_signature_to_kv(key_id, public_key_obj)
                    return True
            ...
            if isinstance(activity, Activity):
                ...
                if public_key:
                    ...
                    try:
                        ...
                        if not cache:
                            await self.__save_signature_to_kv(key_id, public_key)
                        return True
    ...
    async def __verify_ld(self, body: bytes, no_check_cache: bool = False) -> bool:
        ...
        if isinstance(activity, Activity):
            ...
            if public_key and creator:
                try:
                    public_key = (
                        public_key if not isinstance(public_key, str) else public_key
                    )
                    if public_key and isinstance(public_key, RSAPublicKey):
                        ld.verify(body_json, public_key, raise_on_fail=True)
                        if not cache:
                            await self.__save_signature_to_kv(creator, public_key)
                        return True
                    raise VerificationFailedError("publicKey does not exist.")
                except (
                    UnknownSignatureError,
